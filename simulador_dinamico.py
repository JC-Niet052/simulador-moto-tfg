...
with tab_susp:
    ...
    K_horq_single = st.number_input("K Horquilla (N/mm por barra)", value=7.5)
    K_horq = K_horq_single * 2.0 # 2 barras
    K_amort = st.number_input("K Amortiguador (N/mm)", value=85.0)
    
    pre_horq = st.number_input("Precarga Horquilla (mm)", value=15.0)
    pre_amort = st.number_input("Precarga Amortiguador (mm)", value=8.0)

    st.divider()
    st.markdown("**🏎️ Load Transfer (ZeroChassis Mode)**")
    ...
    recorrido_max_horq = 120.0
    recorrido_max_amort = 60.0
...
delta_W = W_total * acel_g * (abs(h_cdg) / p_estatica_base)
...
f_axial_horq = W_front_dyn * math.cos(math.radians(24.0)) 
...
st.markdown("##### 🏎️ Telemetría de Masas Dinámica (ZeroChassis Engine)")
