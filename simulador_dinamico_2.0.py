...
st.title("TFG Juan Carlos Nieto")
st.markdown("#### Software de análisis dinámico de geometrías en motocicletas.")
st.markdown("""
<style>
    .block-container { padding-top: 2.5rem; padding-bottom: 0.5rem; max-width: 1050px; }
    ...
    section[data-testid="stSidebar"] h3, section[data-testid="stSidebar"] h4 { font-size: 12px !important; }
    section[data-testid="stSidebar"] .stNumberInput input { font-size: 11px !important; padding: 1px 4px !important; }
    section[data-testid="stSidebar"] .stSlider { margin-bottom: 4px !important; }
    section[data-testid="stSidebar"] .stMarkdown p { margin-bottom: 2px !important; }
    h5 { font-size: 13px !important; margin-bottom: 4px !important; }
...
plantilla = st.sidebar.selectbox("🏍️ Cargar Moto", ["Yamaha YZF-R6 (2004)", "MIR Racing Moto5 (2020)"])

if plantilla == "Yamaha YZF-R6 (2004)":
    p_est_A = 1380.0; L_b_A = 577.0; rake_A = 24.0; pipa_A = 180.0; off_A = 35.0; gr_A = 25.0
    h_am_A = 172.0; d_am_A = 75.0; h_lk_A = -156.0; d_lk_A = 40.0
    pipa_B = 180.0; d_pt = 696.791; d_pb = 658.423; d_pa = 187.641; d_ta = 656.397; d_pl = 161.047; d_bl = 777.448; d_al = 329.862
    off_B = 35.0; gr_B = 25.0
    lb_p = 202.5; hb_p = 60.0; Ll = 150.0; d_ba = 98.0; d_bl_tri = 79.0; d_la = 112.0
    L_h = 710.0; L_am = 295.0
    kh = 9.5; ka = 95.0; ph = 15.0; pa = 8.0 
    wf = 120.0; pf = 70.0; rf = 17.0; wr = 180.0; pr = 55.0; rr = 17.0
    pm = 162.0; xm = 220.0; ym = 180.0; pp = 75.0; xp = 100.0; yp = 450.0
else:
    p_est_A = 1263.46; L_b_A = 561.5; rake_A = 22.19; pipa_A = 139.0; off_A = 33.5; gr_A = 19.5
    h_am_A = 125.70; d_am_A = 46.49; h_lk_A = -131.69; d_lk_A = 14.59
    pipa_B = 139.0; d_pt = 593.05; d_pb = 568.022; d_pa = 134.02; d_ta = 563.94; d_pl = 132.5; d_bl = 648.135; d_al = 259.361
    off_B = 33.5; gr_B = 19.5
    lb_p = 170.0; hb_p = 46.0; Ll = 168.0; d_ba = 115.0; d_bl_tri = 68.0; d_la = 95.5
    L_h = 625.0; L_am = 314.0
    kh = 7.5; ka = 85.0; ph = 15.0; pa = 8.0
    wf = 90.0; pf = 58.0; rf = 17.0; wr = 115.0; pr = 75.0; rr = 17.0
    pm = 85.0; xm = 200.0; ym = 150.0; pp = 60.0; xp = 80.0; yp = 450.0
...
with tab_ruedas:
    st.markdown("**Neumático Delantero**")
    w_f = st.number_input("Ancho (mm) Del.", value=float(wf))
    p_f = st.number_input("Perfil (%) Del.", value=float(pf))
    r_f = st.number_input("Llanta (Pulgadas) Del.", value=float(rf))
    Df = (r_f * 25.4) + 2.0 * (w_f * (p_f / 100.0))
    

    st.markdown("**Neumático Trasero**")
    w_r = st.number_input("Ancho (mm) Tras.", value=float(wr))
    p_r = st.number_input("Perfil (%) Tras.", value=float(pr))
    r_r = st.number_input("Llanta (Pulgadas) Tras.", value=float(rr))
...
with tab_chasis:
    ...
    if metodo_chasis == "A: Comercial (Wheelbase, Rake)":
        ...
        p_estatica_input = st.number_input("Batalla Comercial (Ficha Técnica)", value=float(p_est_A), format="%.1f")
        L_basc_ficha = st.number_input("Basculante Comercial (Ficha Técnica)", value=float(L_b_A), format="%.1f")
        ang_lanz_input = st.number_input("Ángulo Rake Base", value=float(rake_A), format="%.2f")
        L_pipa_input = st.number_input("Longitud Pipa Dirección", value=float(pipa_A), format="%.1f")
        
        st.divider()
        d_offset = st.number_input("Offset Tijas", value=float(off_A), format="%.1f")
        grosor_tija = st.number_input("Grosor Tija Superior", value=float(gr_A), format="%.1f")
        
        st.markdown("**Anclajes Traseros (Coords Clásicas Locales)**")
        H_amort = st.number_input("Y Pivot a Amortiguador Sup.", value=float(h_am_A), format="%.2f")
        d_amort = st.number_input("X Soporte Amortiguador (d_amort)", value=float(d_am_A), format="%.2f")
        H_link = st.number_input("Y Pivot a Bieleta", value=float(h_lk_A), format="%.2f")
        d_link = st.number_input("X Soporte Bieleta (d_link)", value=float(d_lk_A), format="%.2f")
    else:
        
        L_pipa_input = st.number_input("Longitud Pipa Dirección", value=int(pipa_B))
        d_piv_top = st.number_input("Distancia eje PIVOT a Pipa SUPERIOR", value=float(d_pt))
        d_piv_bot = st.number_input("Distancia eje PIVOT a Pipa INFERIOR", value=float(d_pb), format="%.3f")

        st.divider()
        d_offset = st.number_input("Offset Tijas", value=float(off_B))
        grosor_tija = st.number_input("Grosor Tija Superior", value=float(gr_B))
        
        st.markdown("**Anclajes Traseros (Multilateración)**")
        d_piv_amort = st.number_input("Distancia PIVOT a Soporte AMORT.", value=float(d_pa), format="%.3f")
        d_top_amort = st.number_input("Distancia Pipa SUP a Soporte AMORT.", value=float(d_ta), format="%.3f")
        d_piv_link = st.number_input("Distancia PIVOT a Soporte BIELETA", value=float(d_pl), format="%.3f")
        d_bot_link = st.number_input("Distancia Pipa INF a Soporte BIELETA", value=float(d_bl), format="%.3f")
        
        
        d_amort_link = st.number_input("Distancia AMORTIGUADOR a BIELETA", value=float(d_al), format="%.3f")

    st.markdown("**Anclajes en el Basculante (Rocker Pivot)**")
    l_basc_piv = st.number_input("X Pivot a Anclaje Bieleta en Basc.", value=float(lb_p))
    h_basc_piv = st.number_input("Y Pivot a Anclaje Bieleta en Basc.", value=float(hb_p))

with tab_susp:
    L_horq_ext = st.number_input("Longitud Horquilla Ext. Total", value=float(L_h))
    L_amort_ext_base = st.number_input("Longitud Amortiguador Ext.", value=float(L_am))
    
    st.markdown("**Recorridos Máximos**")
    col_rh, col_ra = st.columns(2)
    recorrido_max_horq = col_rh.number_input("Recorrido Máx. Horquilla (mm)", value=120.0, min_value=10.0, max_value=300.0, step=5.0)
    recorrido_max_amort = col_ra.number_input("Recorrido Máx. Amortiguador (mm)", value=60.0, min_value=5.0, max_value=150.0, step=2.5)
    ...
    st.divider()
    modo_suspension = st.radio("Modo de Simulación:", ["Dinámico (Fuerzas G)", "Manual (Directo)"])
    
    if modo_suspension == "Dinámico (Fuerzas G)":
        st.markdown("**🔧 Constantes Elásticas (Muelles)**")
        # Quitamos estos inputs redundantes y usamos los de Setup Horquilla/Amortiguador que son los que el usuario prefiere
        st.info("Ajusta los muelles en las secciones 'Setup Horquilla' y 'Setup Amortiguador' más abajo.")
        K_horq = 0.0 # Se calculará abajo
        K_amort = 0.0 # Se calculará abajo
        pre_horq = 0.0
        pre_amort = 0.0
        st.divider()
        st.markdown("**🏎️ Load Transfer (Load Transfer Mod)**")
        acel_g = st.slider("Fuerza G Longitudinal", -1.5, 1.5, 0.0, 0.1, help="-G equivale a frenada (hundimiento eje delantero). +G equivale a aceleración.")
        c_horq_manual = 0.0
        c_amort_manual = 0.0
    else:
        st.markdown("**🎚️ Compresión Manual**")
        
        st.markdown("Horquilla")
        col_s1, col_n1 = st.columns([3, 1])
        _slider_horq = col_s1.slider("##horq_sl", 0.0, float(recorrido_max_horq), 0.0, label_visibility="collapsed")
        _input_horq = col_n1.number_input("mm##horq", 0.0, float(recorrido_max_horq), _slider_horq, step=0.5, label_visibility="collapsed")
        c_horq_manual = _input_horq

        st.markdown("Amortiguador")
        col_s2, col_n2 = st.columns([3, 1])
        _slider_amort = col_s2.slider("##amort_sl", 0.0, float(recorrido_max_amort), 0.0, label_visibility="collapsed")
        _input_amort = col_n2.number_input("mm##amort", 0.0, float(recorrido_max_amort), _slider_amort, step=0.5, label_visibility="collapsed")
        c_amort_manual = _input_amort
        
        K_horq = 0.0; K_amort = 0.0; pre_horq = 0.0; pre_amort = 0.0; acel_g = 0.0


    st.divider()
    st.markdown("**Setup Chasis**")
    st.caption("HP Insert: ajusta longitud y ángulo del amortiguador. Pivot: desplaza el eje del basculante.")
    col_c1, col_c2 = st.columns(2)
    hp_insert_mm  = col_c1.number_input("HP Insert Δmm", value=0.0, step=0.5, format="%.1f", key="hp_mm",
                                          help="Variación de longitud del amortiguador (± mm)")
    hp_insert_deg = col_c2.number_input("HP Insert Δ°", value=0.0, step=0.5, format="%.1f", key="hp_deg",
                                         help="Variación de ángulo de rake inducido por el HP Insert (± grados)")
    pivot_delta_y = col_c1.number_input("Pivot ΔY (mm)", value=0.0, step=0.5, format="%.1f", key="piv_dy",
                                         help="Desplazamiento vertical del pivote del basculante")
    pivot_delta_x = col_c2.number_input("Pivot ΔX (mm)", value=0.0, step=0.5, format="%.1f", key="piv_dx",
                                         help="Desplazamiento horizontal del pivote del basculante")
    lean_angle    = col_c1.number_input("Lean Angle (°)", value=0.0, step=0.5, format="%.1f", key="lean_ang",
                                         help="Ángulo de inclinación en curva (informativo)")

    st.markdown("**Setup Horquilla**")
    col_h1, col_h2 = st.columns(2)
    horq_oil    = col_h1.number_input("Aceite (mm)", value=240, step=5, key="horq_oil")
    horq_spring = col_h2.number_input("Muelle (N/mm)", value=float(kh), step=0.5, key="horq_spring_input", help="Sincronizado con pestaña Suspensiones")
    # Sincronizamos con el valor de la pestaña suspensiones si se desea, o lo dejamos como está
    # Para paridad total, usamos horq_spring_input como base si modo_suspension == "Dinámico"
    if modo_suspension == "Dinámico (Fuerzas G)":
        K_horq = horq_spring * 2.0
    horq_pre    = col_h1.number_input("Precarga (mm)", value=float(ph), step=0.5, key="horq_pre2")
    st.caption("TOS (contramuelle) → Rate × Longitud (N/mm × mm)")
    col_ht1, col_ht2 = st.columns(2)
    horq_tos_rate = col_ht1.number_input("TOS Rate (N/mm)", value=0.0, step=0.5, format="%.1f", key="h_tos_rate", help="Rigidez del contramuelle")
    horq_tos_len  = col_ht2.number_input("TOS Long. (mm)",  value=0.0, step=1.0, format="%.0f", key="h_tos_len",  help="Longitud del contramuelle")
    st.caption("BOS (tope final de carrera) → Rate × Longitud")
    col_hb1, col_hb2 = st.columns(2)
    horq_bos_rate = col_hb1.number_input("BOS Rate (N/mm)", value=0.0, step=0.5, format="%.1f", key="h_bos_rate")
    horq_bos_len  = col_hb2.number_input("BOS Long. (mm)",  value=0.0, step=1.0, format="%.0f", key="h_bos_len")

    st.markdown("**Setup Amortiguador**")
    col_s1, col_s2 = st.columns(2)
    amort_spring = col_s1.number_input("Muelle (N/mm)", value=float(ka), step=5.0, key="amort_spring_input")
    if modo_suspension == "Dinámico (Fuerzas G)":
        K_amort = amort_spring
    amort_pre    = col_s2.number_input("Precarga (mm)", value=float(pa), step=0.5, key="amort_pre2_input")
    if modo_suspension == "Dinámico (Fuerzas G)":
        pre_amort = amort_pre
    st.caption("TOS (contramuelle) → Rate × Longitud")
    col_st1, col_st2 = st.columns(2)
    amort_tos_rate = col_st1.number_input("TOS Rate (N/mm)", value=0.0, step=0.5, format="%.1f", key="s_tos_rate")
    amort_tos_len  = col_st2.number_input("TOS Long. (mm)",  value=0.0, step=1.0, format="%.0f", key="s_tos_len")
    amort_bos_rate = 0.0  # Los amortiguadores traseros no llevan BOS
    amort_bos_len  = 0.0

    st.divider()
    st.markdown("**Geometría de Bieletas (Rocker y Tirante)**")
    L_link = st.number_input("Longitud del Tirante (Pullrod)", value=float(Ll))
    d_t_ba = st.number_input("Triángulo: Basc. a Amortiguador", value=float(d_ba))
    d_t_bl = st.number_input("Triángulo: Basc. a Tirante", value=float(d_bl_tri))
    d_t_la = st.number_input("Triángulo: Tirante a Amortiguador", value=float(d_la))
...
with tab_cdg:
    ...
    if modo_cdg == "Aporte Manual Combinado":
        ...
        d_cdg = st.number_input("X Pivot a CDG (Horizontal)", value=float(ym)) 
        h_cdg = st.number_input("Y Pivot a CDG (Vertical)", value=135.0)
    else:
        st.markdown("**🔹 Vehículo**")
        peso_moto = st.number_input("Peso Moto (kg)", value=float(pm))
        x_moto = st.number_input("X Pivot a CDG Moto", value=float(xm))
        y_moto = st.number_input("Y Pivot a CDG Moto", value=float(ym))
        
        st.markdown("**🔹 Piloto**")
        peso_piloto = st.number_input("Peso Piloto (kg)", value=float(pp))
        x_piloto = st.number_input("X Pivot a CDG Piloto", value=float(xp))
        y_piloto = st.number_input("Y Pivot a CDG Piloto", value=float(yp))
...
with tab_trans:
    ...
    d_pinon = st.number_input("X Pivot a Piñón", value=float(pm)) 
...
# ==========================================
Pivot_local = np.array([-float(pivot_delta_x), float(pivot_delta_y)])  # X+: closer to rear wheel; Y+: upward
# HP Insert adjustments: mm shifts amortiguador length, degrees shifts rake
L_amort_ext = L_amort_ext_base + float(hp_insert_mm)
hp_rake_delta = float(hp_insert_deg)  # extra rake degrees induced by HP Insert (applied below)

if metodo_chasis == "A: Comercial (Wheelbase, Rake)":
    F_Amort_local = np.array([-float(d_amort), float(H_amort)])
    F_Link_local = np.array([-float(d_link), float(H_link)])
else:
    # 1. Construcción local pura respecto a un pivote temporal en (0,0)
    pivot_tmp = np.array([0.0, 0.0])
    bot_pipa_tmp = np.array([d_piv_bot, 0.0])
    top_pipa_tmp = intersect_circles(pivot_tmp, d_piv_top, bot_pipa_tmp, L_pipa_input, pick_highest_y=True)
    
    if top_pipa_tmp is None:
        st.error("❌ Geometría de Pipa rota.")
        st.stop()

    # 2. Normalización al horizonte respecto al pivote temporal
    v_pipa_ini = top_pipa_tmp - bot_pipa_tmp
    ang_actual = math.atan2(v_pipa_ini[1], v_pipa_ini[0])
    target_pipa_angle = math.radians(90.0 + rake_A)
    theta_norm = target_pipa_angle - ang_actual
    ...
    top_pipa_norm = np.dot(mat_norm, top_pipa_tmp)
    bot_pipa_norm = np.dot(mat_norm, bot_pipa_tmp)
    pivot_norm = np.dot(mat_norm, pivot_tmp) # Sigue siendo 0,0

    # 3. Buscar anclajes traseros respecto al pivote temporal (0,0)
    opt_a1, opt_a2 = intersect_circles_all(pivot_norm, d_piv_amort, top_pipa_norm, d_top_amort)
    opt_b1, opt_b2 = intersect_circles_all(pivot_norm, d_piv_link, bot_pipa_norm, d_bot_link)
    ...
    if best_pair is None:
        ...
        # 4. APLICAMOS EL DESPLAZAMIENTO DEL PIVOTE A TODOS LOS PUNTOS AL FINAL
        top_pipa_local_B = top_pipa_norm + Pivot_local
        bot_pipa_local_B = bot_pipa_norm + Pivot_local
        F_Amort_local, F_Link_local = best_pair
        F_Amort_local = F_Amort_local + Pivot_local
        F_Link_local = F_Link_local + Pivot_local
...
def resolver_Ry_final(L_target):
    Ry_local = resolver_Ry(L_target)
    Rx_local = -math.sqrt(max(0, L_basc**2 - Ry_local**2))
    return np.array([Rx_local + Pivot_local[0], Ry_local + Pivot_local[1]])
...
if metodo_chasis == "A: Comercial (Wheelbase, Rake)":
    ...
    top_pipa_local = (tija_base_stat - d_offset * v_fwd_local) + Pivot_local
    bot_pipa_local = (top_pipa_local - L_pipa_input * v_up_local)
    
    chasis_sup_local = np.array([0, float(H_amort)]) + Pivot_local
    chasis_inf_local = np.array([0, float(H_link)]) + Pivot_local
else:
    ...
    # Definimos estructura ortogonal idéntica al Método A para estética coherente
    chasis_sup_local = np.array([Pivot_local[0], F_Amort_local[1]])
    chasis_inf_local = np.array([Pivot_local[0], F_Link_local[1]])

v_up = v_up_local # Alias para la Fase 9
centro_pipa_local = (top_pipa_local + bot_pipa_local) / 2 # type: ignore

# ==========================================
# PRE-ENSAMBLAJE CINEMÁTICO REAL LOCAL (ESTÁTICO)
# ==========================================
# Definimos estática para wheelbase y h_pivot
R_local_stat_abs = resolver_Ry_final(L_amort_ext)
R_local_stat = R_local_stat_abs - Pivot_local

h_pivot = radio_tras - R_local_stat_abs[1]
# R_local anterior era relativo [Rx, Ry]
R_local = R_local_stat 

sup_amort_local = F_Amort_local
inf_biel_local = F_Link_local

# ==========================================
# MÓDULO ZEROCHASSIS: LOAD TRANSFER & SAG DINÁMICO
# ==========================================
top_fork_local = tija_sup_face_stat + deslizamiento_horquilla * v_up_local
F_local_stat = top_fork_local - L_horq_ext * v_up_local

ds_x_base = F_local_stat[0] - R_local[0]
ds_y_base = F_local_stat[1] - R_local[1]
p_estatica_base = math.sqrt(max(0.001, ds_x_base**2 + ds_y_base**2))

W_total = masa_total * 9.81
ratio_peso_F = abs(d_cdg - R_local[0]) / p_estatica_base
ratio_peso_R = 1.0 - ratio_peso_F

W_front_0G = W_total * ratio_peso_F
W_rear_0G  = W_total * ratio_peso_R

h_cdg_from_ground = h_cdg + h_pivot  # Altura real CDG desde el asfalto (CoG Y)
delta_W = W_total * acel_g * (h_cdg_from_ground / p_estatica_base)  # Load_transfer = Force × (CoG Y / Wheelbase)
W_front_dyn = max(0.0, W_front_0G - delta_W)
W_rear_dyn = max(0.0, W_rear_0G + delta_W)

Ry_1 = R_local[1]
L_1 = evaluar_amortiguador(Ry_1) # type: ignore
Ry_2 = R_local[1] + 10.0 
L_2 = evaluar_amortiguador(Ry_2) # type: ignore
if L_1 is not None and L_2 is not None:
    MR_medio = abs(L_1 - L_2) / 10.0 
else:
    MR_medio = 0.5
MR_medio = max(0.01, MR_medio)

if metodo_chasis == "A: Comercial (Wheelbase, Rake)":
    rake_for_force = ang_lanz_input
else:
    # En método B, ang_pipa_local (rad) es el lanzamiento
    rake_for_force = math.degrees(ang_pipa_local)

f_axial_horq = W_front_dyn * math.cos(math.radians(rake_for_force)) 
f_axial_amort = W_rear_dyn / MR_medio if MR_medio > 0 else 0

# TOS / BOS efectivo: endurece el muelle al final del recorrido

if modo_suspension == "Dinámico (Fuerzas G)":
    c_horq_total = (f_axial_horq / K_horq) if K_horq > 0 else 0
    c_horq_calc = max(0.0, c_horq_total - pre_horq)
    c_amort_total = (f_axial_amort / K_amort) if K_amort > 0 else 0
    c_amort_calc = max(0.0, c_amort_total - pre_amort)
    
    c_horq = min(c_horq_calc, recorrido_max_horq)
    c_amort = min(c_amort_calc, recorrido_max_amort)
else:
    c_horq = c_horq_manual
    c_amort = c_amort_manual

# TOS / BOS: rigidez efectiva según posición en el recorrido
def effective_K(k_base, compression, tos_rate, tos_len, bos_rate, bos_len, max_travel):
    k_eff = k_base
    if tos_len > 0 and compression >= (max_travel - tos_len):
        k_eff += tos_rate
    if bos_len > 0 and compression >= (max_travel - bos_len):
        k_eff += bos_rate
    return max(k_eff, k_base)

K_horq_eff  = effective_K(K_horq,  c_horq,  horq_tos_rate,  horq_tos_len,  horq_bos_rate,  horq_bos_len,  recorrido_max_horq)
K_amort_eff = effective_K(K_amort, c_amort, amort_tos_rate, amort_tos_len, amort_bos_rate, amort_bos_len, recorrido_max_amort)

# CALCULAMOS POSICIÓN COMPRIMIDA REAL
R_local_comp_abs = resolver_Ry_final(L_amort_ext - c_amort)
R_local_comp = R_local_comp_abs - Pivot_local

L_horq_comp = L_horq_ext - c_horq
F_local = top_fork_local - L_horq_comp * v_up_local

Ry_comp = R_local_comp[1]
Rx_comp = R_local_comp[0]

# ==========================================
# ROTACIÓN GLOBAL HACIA EL SUELO (EL MILAGRO FÍSICO)
# ==========================================
delta_x = F_local[0] - R_local_comp_abs[0] # type: ignore
delta_y = F_local[1] - R_local_comp_abs[1] # type: ignore
...
Pivot_world = Pivot_local # El pivot ya no es 0,0 si hay offset
R_world = np.dot(matriz_rot, R_local_comp_abs)
...
angulo_basc = 180.0 + math.degrees(math.atan2(R_world[1] - Pivot_world[1], R_world[0] - Pivot_world[0]))
...
# Variables previas genéricas
W_front_pct = (W_front_dyn/W_total)*100 if W_total > 0 else 0
W_rear_pct = (W_rear_dyn/W_total)*100 if W_total > 0 else 0
spr_ratio = Z_corona / Z_pinon if Z_pinon > 0 else 0

delta_front_pct = ((W_front_dyn - W_front_0G)/W_total)*100 if W_total > 0 else 0
delta_rear_pct = ((W_rear_dyn - W_rear_0G)/W_total)*100 if W_total > 0 else 0
delta_p = p_dinamico - p_estatica

st.divider()
ver_comparativa = st.checkbox("📊 Modo Analista: Mostrar Comparativa Completa (Sistema vs Dinámico)", value=False)

if ver_comparativa:
    st.markdown("##### 🏎️ Telemetría de Masas Dinámica (Motor Cinemático)")
    col_m1, col_m2, col_m3, col_m4 = st.columns(4)
    col_m1.metric("Peso Front (%)", f"{W_front_pct:.1f} %", f"{delta_front_pct:+.1f}% Load", delta_color="inverse")
    col_m2.metric("Peso Rear (%)", f"{W_rear_pct:.1f} %", f"{delta_rear_pct:+.1f}% Load", delta_color="normal")
    col_m3.metric("Hundimiento Horquilla", f"{c_horq:.1f} mm", f"{f_axial_horq:.0f} N Eje", delta_color="inverse")
    col_m4.metric("Hundimiento Amortiguador", f"{c_amort:.1f} mm", f"{f_axial_amort:.0f} N Eje", delta_color="inverse")
    ...
    # Corrección de coordenadas absolutas de Sistema
    cog_x_zc = cdg_world[0] - R_world[0]  # Respecto al eje trasero (X=0)
    cog_y_zc = cdg_world[1] - suelo_y     # Respecto al nivel del suelo (Y=0)
    front_height_ref = top_fork_world[1] - suelo_y # Altura de la tija sobre el suelo
    ...
    .zc-table tr:nth-child(even) td:last-child {{
        background-color: #1e1e1e;
    }}
    .zc-table tr:hover td {{
    ...
    <div class="zc-wrapper">
        <table class="zc-table">
            <tr>
                <th colspan="2">Chassis Analysis [PRO]</th>
            </tr>
            <tr><td>Rake</td><td><span class="zc-value">{ang_lanz_dinamico:.2f}</span><span class="zc-unit">°</span></td></tr>
            <tr><td>Trail</td><td><span class="zc-value">{trail_real:.1f}</span><span class="zc-unit">mm</span></td></tr>
            <tr><td>Wheelbase</td><td><span class="zc-value">{p_dinamico:.1f}</span><span class="zc-unit">mm</span></td></tr>
            <tr><td>Sw.A angle</td><td><span class="zc-value">{angulo_basc:.2f}</span><span class="zc-unit">°</span></td></tr>
            <tr><td>Front height ref</td><td><span class="zc-value">{front_height_ref:.1f}</span><span class="zc-unit">mm</span></td></tr>
            <tr><td>Fork travel</td><td><span class="zc-value">{c_horq:.1f}</span><span class="zc-unit">mm</span></td></tr>
            <tr><td>Shock travel</td><td><span class="zc-value">{c_amort:.1f}</span><span class="zc-unit">mm</span></td></tr>
            <tr><td>Rw travel</td><td><span class="zc-value">{recorrido_rueda:.1f}</span><span class="zc-unit">mm</span></td></tr>
            <tr><td>CoG X mm</td><td><span class="zc-value">{cog_x_zc:.1f}</span><span class="zc-unit">mm</span></td></tr>
            <tr><td>CoG Y mm</td><td><span class="zc-value">{cog_y_zc:.1f}</span><span class="zc-unit">mm</span></td></tr>
            <tr><td>Anti-squat ang.</td><td><span class="zc-value">{angulo_antisquat:.2f}</span><span class="zc-unit">°</span></td></tr>
            <tr><td>Anti-squat %</td><td><span class="zc-value" style="color: #ffcc00;">{percent_antisquat:.1f}</span><span class="zc-unit">%</span></td></tr>
            <tr><td>F weight %</td><td><span class="zc-value">{W_front_pct:.1f}</span><span class="zc-unit">%</span></td></tr>
            <tr><td>R weight %</td><td><span class="zc-value">{W_rear_pct:.1f}</span><span class="zc-unit">%</span></td></tr>
        </table>
    </div>
    """
    ...
    html_table_2 = f"""
    <div class="zc-wrapper" style="background: none; border: none; box-shadow: none; padding: 0;">
        <span style="font-family: 'Inter', Arial; font-weight: bold; font-size: 13px; margin-top: 15px; display: block; color: #aaa; text-transform: uppercase;">Cinemática de Bieleta</span>
        <table class="zc-table" style="margin-top: 5px;">
            <tr><td>h_pivot (Geométrico)</td><td><span class="zc-value">{h_pivot:.1f}</span><span class="zc-unit">mm</span></td></tr>
            <tr><td>L. Amortiguador</td><td><span class="zc-value">{L_amort_actual:.1f}</span><span class="zc-unit">mm</span></td></tr>
            <tr><td>Recorrido Rueda (Eje Y)</td><td><span class="zc-value">{recorrido_rueda:.1f}</span><span class="zc-unit">mm</span></td></tr>
            <tr><td>Motion Ratio</td><td><span class="zc-value" style="color: #ff6600;">{motion_ratio:.2f}</span></td></tr>
        </table>
    </div>
    """
...
# Chasis (Inamovible)
# Eje estructural principal partiendo del Pivot
ax.plot([Pivot_world[0], chasis_sup_world[0]], [Pivot_world[1], chasis_sup_world[1]], color='navy', lw=1.0)
ax.plot([Pivot_world[0], chasis_inf_world[0]], [Pivot_world[1], chasis_inf_world[1]], color='navy', lw=1.0)
...
ax.plot([top_corona[0], top_pinon[0]], [top_corona[1], top_pinon[1]], color='goldenrod', lw=1.0)
ax.plot([bot_corona[0], bot_pinon[0]], [bot_corona[1], bot_pinon[1]], color='goldenrod', lw=1.0)

# --- GEOMETRÍA DIRECCIÓN (RAKE Y TRAIL) ---
# Eje de dirección
ax.plot([centro_pipa_world[0], steer_ground_x], [centro_pipa_world[1], suelo_y], color='purple', ls='-.', lw=1.0)

# Trail (Elevamos la cota 45 mm del suelo para que la línea negra no lo tape)
y_cota = suelo_y + 45
ax.plot([F_world[0], F_world[0]], [suelo_y, y_cota + 15], color='deepskyblue', ls=':', lw=1.0)
ax.plot([steer_ground_x, steer_ground_x], [suelo_y, y_cota + 15], color='deepskyblue', ls=':', lw=1.0)
ax.plot([F_world[0], steer_ground_x], [y_cota, y_cota], color='deepskyblue', lw=1.5, label='Trail')
ax.plot([F_world[0], F_world[0]], [y_cota - 15, y_cota + 15], color='deepskyblue', lw=1.0)
ax.plot([steer_ground_x, steer_ground_x], [y_cota - 15, y_cota + 15], color='deepskyblue', lw=1.0)

# Dibujo del Ángulo de Rake en el suelo
ax.plot([steer_ground_x, steer_ground_x], [suelo_y, suelo_y + 300], color='purple', ls=':', lw=1.0)
radio_arco = 200.0
...
ax.plot(arc_x, arc_y, color='purple', lw=1.5, label='Rake')

# --- ANÁLISIS ANTI-SQUAT ---
if ic_point is not None:
    # Proyección punteada anti-squat solo desde el piñón
    ax.plot([top_pinon[0], ic_point[0]], [top_pinon[1], ic_point[1]], color='goldenrod', ls='--', lw=1.0)
    ax.plot([R_world[0], ic_point[0]], [R_world[1], ic_point[1]], color='red', ls='--', lw=1.0)
    ax.plot(ic_point[0], ic_point[1], 'mx', markersize=8, label='I.C.')
    
    ax.plot([contacto_trasero_x, ic_point[0]], [contacto_trasero_y, ic_point[1]], color='magenta', ls='-', lw=1.5, label='Ángulo Anti-Squat')
    # Arco visual del ángulo Anti-Squat en el punto de contacto trasero
    _ang_as = math.atan2(ic_point[1] - contacto_trasero_y, ic_point[0] - contacto_trasero_x)
    _ang_ref = 0.0  # horizontal de referencia
    _r_as = 120.0
    _tvals = np.linspace(_ang_ref, _ang_as, 20)
    ax.plot(contacto_trasero_x + _r_as*np.cos(_tvals), contacto_trasero_y + _r_as*np.sin(_tvals), color='magenta', lw=1.0)
    
    ax.plot([contacto_trasero_x, cdg_world[0]], [contacto_trasero_y, cdg_world[1]], color='gray', ls=':', lw=1.0)
...
ax.set_aspect('equal')
ax.legend(loc='upper right', fontsize=7, markerscale=0.8, framealpha=0.7)
...
if st.checkbox("Analizar Curva de Motion Ratio (Progresividad)", value=False):
    ...
    fig_mr, ax_mr = plt.subplots(figsize=(9, 3.5))
